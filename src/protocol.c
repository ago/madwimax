...
int process_response(struct wimax_dev_status *dev, const unsigned char *buf, int len)
{
	...
	if(len < 4) {
		wmlog_msg(0, "short read");
		return -1;
	}

	if(buf[0] != 0x57) {
		wmlog_msg(0, "bad header");
		return -1;
	}
	...
	if(check_len != len) {
		wmlog_msg(0, "bad length: %02x instead of %02x", check_len, len);
		return -1;
	}
	...
	switch (buf[1]) {
		...
		default:
			wmlog_msg(0, "bad response type: %02x", buf[1]);
			return -1;
	}
}
